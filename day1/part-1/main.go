...
// pairLists pairs the lists based
// on the the rule that the lowest numbers from both lists are used
// and paired together
func pairLists() {
	ll := listLeft
	rl := listRight
	...
	for len(ll) > 0 {
		...
		pairedList = append(pairedList, Pair{l, r})
	}
}

func getLowest(s []int) ([]int, int) {
	lowest := s[0]
	lk := 0
	for k, i := range s {
		if i < lowest {
